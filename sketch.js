...
function setup() {
  ...
  for (let i = 0; i < 7; i++) ribbons.push(new Ribbon((i + 0.5) * (width / 7)));
}
...
function draw() {
  ...
  if (random() < 0.015) gifts.push(new Gift(random(width), random(-300, -50)));
  for (let i = gifts.length - 1; i >= 0; i--) {
    gifts[i].update();
    gifts[i].display();
    if (gifts[i].y > height - 55) gifts[i].land();
    if (gifts[i].offscreen()) gifts.splice(i, 1);
  }
  ...
  text(displayText, width / 2, height / 2 - 120);

  drawCake(width / 2, height - 80);
  drawTable(width / 2, height - 20);
...
class Gift {
  ...
  update() {
    ...
    if (this.isGold) {
      this.bri = 90 + sin(frameCount * 0.15) * 10;
    }
  }
...
function drawCake(x, y) {
  ...
  fill(50, 10, 100); rect(-110, 0, 220, 60, 12);
  fill(25, 25, 100); rect(-85, -50, 170, 50, 10);
  fill(40, 20, 100); rect(-60, -85, 120, 35, 8);
  fill(15, 30, 100); for (let i = -55; i <= 55; i += 18) ellipse(i, -87, 10, 10);
  for (let i = -70; i <= 70; i += 35) { fill(randomWarmHue(), 80, 100); ellipse(i, -25, 10); }
  fill(0, 0, 100); rect(-10, -130, 20, 45, 4);
  fill(random(20, 40), 100, 100); ellipse(0, -138, 15, 25);
  pop();
}
...
function drawTable(x, y) {
  push(); translate(x, y); noStroke();
  fill(30, 60, 40);
  rect(-450, 0, 900, 60, 10);
  pop();
}


class Ribbon {
  constructor(x) { this.x = x; this.waveOffset = random(1000); }
  display() {
...
class Confetti {
  constructor() {
    ...
    this.size = random(3, 6); this.baseHue = randomWarmHue();
    this.speed = random(1, 3); this.angle = random(TWO_PI);
    this.spin = random(-0.08, 0.08); this.offset = random(1000);
  }
  update() {
    this.y += this.speed; this.angle += this.spin; this.x += sin(this.angle) * 0.5;
    if (this.y > height + 10) { this.y = random(-100, -10); this.x = random(width); this.baseHue = randomWarmHue(); }
  }
  display() {
    let hueShift = map(sin(frameCount * 0.04 + this.offset), -1, 1, -20, 20);
    let hueNow = (this.baseHue + hueShift + 360) % 360;
    push(); translate(this.x, this.y); rotate(this.angle);
    fill(hueNow, 80, 100); rect(0, 0, this.size * 2, this.size / 2); pop();
  }
}
...
class Firework {
  constructor() {
    this.x = random(width); this.y = height;
    this.hues = [random(15,40), random(330,360), random(45,55), random(25,35)];
    this.particles = []; this.exploded = false; this.speed = random(8,14);
  }
  update() {
    if (!this.exploded) {
      this.y -= this.speed; this.speed -= 0.25;
      if (this.speed <= 0) {
        this.exploded = true;
        for (let i = 0; i < 150; i++) {
  ...
  show() {
    if (!this.exploded) { fill(this.hues[0], 100, 100); ellipse(this.x, this.y, 8); }
    for (let p of this.particles) p.show();
  }
  done() { return this.exploded && this.particles.every(p => p.lifespan < 0); }
}
class Particle {
  constructor(x, y, hue) {
    this.pos = createVector(x, y);
    this.vel = p5.Vector.random2D().mult(random(2, 8));
    this.lifespan = 255; this.hue = hue;
  }
  update() { this.pos.add(this.vel); this.vel.mult(0.94); this.lifespan -= 3; }
  show() { noStroke(); fill(this.hue, 100, 100, this.lifespan); ellipse(this.pos.x, this.pos.y, 5); }
}


function randomWarmHue() { let ranges = [random(330,360), random(15,35), random(40,55)]; return random(ranges); }